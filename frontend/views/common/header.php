<div class="header w1210 bc mt15">
            ...
            <div class="cat_bd <?= \Yii::$app->controller->id . "/" . \Yii::$app->controller->action->id == 'index/index' ? "" : "none" ?>">
                ...
                <?php foreach (\backend\models\Category::find()->where(['parent_id' => 0])->all() as $k1 => $v1): ?>
                    ...
                    <div class="cat <?= $k1 == 0 ? "item1" : "" ?>">
                        <h3><a href="<?= \yii\helpers\Url::to(['goods/lists', 'id' => $v1->id]) ?>"><?= $v1->name ?></a>
                            <b></b></h3>
                        <div class="cat_detail">
                            ...
                            <?php foreach (\backend\models\Category::find()->where(['parent_id' => $v1->id])->all() as $k2 => $v2): ?>
                                <dl class="<?= $k2 == 0 ? "dl_1st" : "" ?>">
                                    <dt>
                                        <a href="<?= \yii\helpers\Url::to(['goods/lists', 'id' => $v2->id]) ?>"><?= $v2->name ?></a>
                                    </dt>
                                    <dd>
    ...
    //缓存名称
    $cacheName = \Yii::$app->controller->id . "/" . \Yii::$app->controller->action->id == 'index/index' ? "index" : "other";
    //判断是否存在缓存
    if (Yii::$app->cache->get($cacheName)) {
        ...
        echo $html;
        //设置缓存
        Yii::$app->cache->set($cacheName, $html, 7 * 3600);
    }
